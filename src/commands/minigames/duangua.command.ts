...
export async function duaLinhThuCommand(message: Message, args: string[]): Promise<void> {
  ...
  const paid = await UserService.deductDongAtomic(userId, betDong);
  if (!paid) {
    await message.reply(`❌ Bạn không đủ ${formatDong(betDong)} để tham gia cuộc đua Linh Thú!`);
    return;
  }
  ...
  await UserService.updateCooldownAtomic(userId, 'dua_linhthu', Date.now());

  const isWin = winner.num === chosenPet;
  const reward = isWin ? betDong * 3 : 0;

  if (isWin) {
    await UserService.addDongAtomic(userId, reward);
  }
  ...
  const embed = createDongSonEmbed()
    ...
    .setDescription(
      `📊 **KẾT QUẢ ĐẠI HỘI ĐUA LINH THÚ:**\n${rankStr}\n\n` +
        (isWin
          ? `🎉 **ĐẠI THẮNG!** Linh thú **#${chosenPet}** của bạn đã xuất sắc về đích **HẠNG 1**! Nhận thưởng **+${formatDong(
              reward
            )}** (x3 cược)!`
          : `💔 **THẤT BẠI!** Linh thú của bạn đã bị vượt mặt bởi **${winner.name}**! Mất ${formatDong(betDong)}.`)
    );
