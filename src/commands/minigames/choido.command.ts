...
export async function choiDoCommand(message: Message): Promise<void> {
  ...
  const cooldownCheck = CooldownEngine.checkCooldown(user, 'choido', 60000);
  ...
  try {
    ...
    if (isCorrect) {
      ...
      await UserService.updateCooldownAtomic(userId, 'choido', Date.now());
