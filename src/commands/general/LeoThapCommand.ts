...
import { renderProgressBar } from '../../utils/formatters';
...
export async function leoThapCommandClean(message: Message): Promise<void> {
  ...
  if (!sessionManager.lock(userId)) {
    await message.reply('⚠️ Bạn đang có một phiên chiến đấu / thao tác chưa hoàn thành!');
    return;
  }
  ...
  if (session.isAwaitingBoon) {
    ...
    collector.on('collect', async (i) => {
      ...
      await i.update({ content: res.message, components: [disabledRow] });
    ...
    collector.on('end', async (_, reason) => {
      ...
      if (reason === 'time') {
        const disabledRow = new ActionRowBuilder<ButtonBuilder>();
        row.components.forEach((btn) => disabledRow.addComponents(ButtonBuilder.from(btn).setDisabled(true)));
        await replyMsg.edit({ content: '⏰ Đã hết 60 giây! Phiên chọn Bùa bị hủy.', components: [disabledRow] });
      }
    });
    return;
  }
  ...
  if (session.isAtRestStation) {
    ...
    collector.on('end', async (_, reason) => {
      sessionManager.unlock(userId);
      if (reason === 'time') {
        await replyMsg.edit({ content: '⏰ Đã hết 60 giây trạm nghỉ.', components: [] });
      }
    });
    return;
  }
  ...
  const activeBuffsStr =
    session.activeBuffs.length > 0
      ? session.activeBuffs.map((b) => `${b.icon} **${b.name}**`).join('\n')
    ...
    .addFields(
      {
        ...
        value:
          `🩸 **Sinh Lực:** \`${session.currentHp.toLocaleString('vi-VN')} / ${session.maxHp.toLocaleString('vi-VN')}\` ${renderAsciiBar(
            session.currentHp,
            session.maxHp
          )}\n` +
  ...
  collector.on('collect', async (i) => {
    if (i.customId === 'tower_retreat') {
      ...
      await i.update({ embeds: [retreatEmbed], components: [disabledRow] });
  ...
  collector.on('end', async (_, reason) => {
    ...
    if (reason === 'time') {
      const disabledRow = new ActionRowBuilder<ButtonBuilder>();
      row.components.forEach((btn) => disabledRow.addComponents(ButtonBuilder.from(btn).setDisabled(true)));
      await replyMsg.edit({ content: '⏰ Đã hết 60 giây! Thao tác leo tháp bị hủy.', components: [disabledRow] });
    }
  });
}
