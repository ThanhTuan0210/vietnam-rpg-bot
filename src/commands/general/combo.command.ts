...
export async function comboAllCommand(message: Message): Promise<void> {
  ...
  await UserService.updateCooldownAtomic(userId, 'san', now);
  await UserService.updateCooldownAtomic(userId, 'don_cui', now);
  await UserService.updateCooldownAtomic(userId, 'dao_khoang', now);
  await UserService.updateCooldownAtomic(userId, 'cau_ca', now);
  await UserService.updateCooldownAtomic(userId, 'hai_thuoc', now);
  ...
  const fishRes = await GatheringService.fish(userId);
  ...
  const woodText = woodRes.itemsGained.map((i: any) => `🪵 **${i.name}** x${i.qty}`).join(', ');
  const mineText = mineRes.itemsGained.map((i: any) => `🪨 **${i.name}** x${i.qty}`).join(', ');
  const fishText = fishRes.itemsGained.map((i: any) => `🎣 **${i.name}** x${i.qty}`).join(', ');
  ...
  const embed = createDongSonEmbed()
    ...
    .setDescription(
      `⚡ **Bạn vừa đồng thời thực hiện 5 công việc lao động & săn bắt dũng mãnh:**\n\n` +
        `⚔️ **SĂN QUÁI (HUNT):** Đả bại **${baseMonster.icon} ${baseMonster.name}** (Gây \`${damageDealt}\` DMG${critBadge})\n` +
        `✨ Thưởng Săn: **+${expEarned} EXP** | **+${formatDong(dongEarned)}** | Loot: ${huntLootText}\n\n` +
