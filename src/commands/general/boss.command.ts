...
import { renderHpBar, renderProgressBar, formatDong } from '../../utils/formatters';

export async function bossCommand(message: Message): Promise<void> {
  ...
  const checkRes = await AreaBossService.checkAndConsumeMeritForBoss(userId, currentArea);
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId('boss_attack').setLabel('⚔️ Tấn Công Sát Thể').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('boss_skill').setLabel('⚡ Kỹ Năng Hệ Phái').setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId('boss_defend').setLabel('🛡️ Phòng Thủ Trấn Trạng').setStyle(ButtonStyle.Secondary)
  );

  const renderBossEmbed = (logs: string[]) => {
      ...
      .addFields(
        {
          ...
          value: `❤️ HP: ${renderHpBar(playerHp, totalStats.totalMaxHp)}\n💧 MP: ${renderProgressBar(
            playerMp,
            totalStats.totalMaxMp,
            10,
            '🟦',
            '⬛'
          )}`,
        ...
        },
        {
          ...
          value: `❤️ HP: ${renderHpBar(bossHp, boss.maxHp)}`,
          inline: true,
        }
      );
  };
  ...
  collector.on('collect', async (i) => {
    ...
    if (bossHp > 0) {
      ...
      if (isDefending) bDmg = Math.floor(bDmg * 0.5);
    ...
    if (bossHp <= 0 || playerHp <= 0) {
      ...
      if (bossHp <= 0) {
        ...
        endEmbed.setDescription(
          `🌟 **BÁO TIN THẮNG TRẬN!** Bạn đã dũng mãnh đánh tan **${boss.icon} ${boss.name}**!\n\n` +
            `🗺️ **RẢNH RỖI TIẾN VÀO:** **Khu Vực ${nextArea}** (Mở khóa Quái mới & Tài nguyên hiếm mới!)\n\n` +
            `🎁 **Phần thưởng:** **+${boss.rewardExp} EXP** | ${formatDong(
              boss.rewardDong
            )} | 💎 **+${boss.rewardKimBao} Kim Bảo**!`
        );
      } else {
