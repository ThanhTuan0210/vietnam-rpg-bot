...
export async function useCommand(message: Message, args: string[]): Promise<void> {
  ...
  if (itemId === 'dan_dot_pha') {
    ...
    await UserModelAdvanced.updateOne({ userId }, { $inc: { 'canhGioi.capDo': 1 } });
  ...
  if (itemId === 'linh_moc_uu_dam') {
    const consumed = await UserService.consumeItemAtomic(userId, 'linh_moc_uu_dam', 1);
    if (!consumed) {
  ...
  if (itemId === 'the_skip_nhiem_vu') {
    ...
    const consumed = await UserService.consumeItemAtomic(userId, 'the_skip_nhiem_vu', 1);
    if (!consumed) {
    ...
    await UserService.updateCooldownAtomic(userId, 'skip_quest', now);
    ...
    await UserService.applyBattleResults(userId, user.chiSo.hp, rewardExp, 0, false, user.canhGioi.capDo, []);

    const embed = createDongSonEmbed()
      .setTitle('📜 HOÀN THÀNH CÁO THỊ THÀNH CÔNG (SKIP)!')
      .setDescription(
        `Bạn đã dùng **Thẻ Bỏ Qua Nhiệm Vụ** và **HOÀN THÀNH THẮNG LỢI TOÀN BỘ CÁO THỊ NHIỆM VỤ!**\n\n` +
          `🎁 **Phần thưởng nhận được:**\n` +
          `• 🪙 **+${formatDong(rewardDong)}**\n` +
          `• 💎 **+${formatKimBao(rewardKimBao)}**\n` +
  ...
  if (itemId === 'com_lam' || itemId === 'life_potion') {
    const consumed = await UserService.consumeItemAtomic(userId, 'com_lam', 1);
    if (!consumed) {
  ...
  if (itemId === 'thuoc_huyet_long') {
    const consumed = await UserService.consumeItemAtomic(userId, 'thuoc_huyet_long', 1);
    if (!consumed) {
  ...
  if (itemId === 'thuoc_linh_khi') {
    const consumed = await UserService.consumeItemAtomic(userId, 'thuoc_linh_khi', 1);
    if (!consumed) {
  ...
  if (itemId === 'dan_cuong_no') {
    const consumed = await UserService.consumeItemAtomic(userId, 'dan_cuong_no', 1);
    if (!consumed) {
  ...
  if (itemId === 'thuoc_kim_cuong') {
    const consumed = await UserService.consumeItemAtomic(userId, 'thuoc_kim_cuong', 1);
    if (!consumed) {
  ...
  if (itemId === 'bua_bao_kich') {
    const consumed = await UserService.consumeItemAtomic(userId, 'bua_bao_kich', 1);
    if (!consumed) {
  ...
  if (itemId === 'bua_chieu_tai') {
    const consumed = await UserService.consumeItemAtomic(userId, 'bua_chieu_tai', 1);
    if (!consumed) {
  ...
  if (itemId === 'bua_hoi_cooldown') {
    const consumed = await UserService.consumeItemAtomic(userId, 'bua_hoi_cooldown', 1);
    if (!consumed) {
    ...
    await UserModelAdvanced.updateOne(
      { userId },
      { $set: { 'cooldowns.san': 0, 'cooldowns.don_cui': 0, 'cooldowns.dao_khoang': 0, 'cooldowns.cau_ca': 0, 'cooldowns.hai_thuoc': 0 } }
    );
    const embed = createDongSonEmbed()
  ...
  if (itemId === 'thuoc_nhan_doi_exp') {
    const consumed = await UserService.consumeItemAtomic(userId, 'thuoc_nhan_doi_exp', 1);
    if (!consumed) {
  ...
  if (itemId === 'thien_linh_dan') {
    const consumed = await UserService.consumeItemAtomic(userId, 'thien_linh_dan', 1);
    if (!consumed) {
  ...
  if (itemId === 'bua_may_man') {
    const consumed = await UserService.consumeItemAtomic(userId, 'bua_may_man', 1);
    if (!consumed) {
