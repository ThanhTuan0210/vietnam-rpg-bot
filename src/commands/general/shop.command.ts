...
export async function shopCommand(message: Message, args: string[]): Promise<void> {
  ...
  const medievalShopCatalog: Record<string, { name: string; price: number; type: string; desc: string }> = {
    // Dược Phép & Hồi Phục
    potion_01a: { name: ITEMS['potion_01a']?.name || 'Thuốc Hồi Máu HP', price: 100, type: 'Bào Chế', desc: 'Hồi phục 100 HP & MP ngay lập tức' },
    potion_02a: { name: ITEMS['potion_02a']?.name || 'Thuốc Hồi Mana MP', price: 150, type: 'Bào Chế', desc: 'Hồi phục 100 MP mana phép thuật' },
    potion_03a: { name: ITEMS['potion_03a']?.name || 'Ma Dược Cuồng Nộ', price: 500, type: 'Bào Chế', desc: 'Tăng +20% Sát Thương (ATK) trong Ngục Tối' },

    // Vũ Khí & Giáp Sơ Cấp Trung Cổ
    sword_01a: { name: ITEMS['sword_01a']?.name || 'Kiếm Sơ Cấp Trung Cổ', price: 1000, type: 'Rèn Đồ', desc: '+15 Sát Thương ATK Kị Sĩ' },
    shield_01a: { name: ITEMS['shield_01a']?.name || 'Khiên Thép Kị Sĩ', price: 1200, type: 'Rèn Đồ', desc: '+10 Phòng Thủ DEF' },
    staff_01a: { name: ITEMS['staff_01a']?.name || 'Trượng Gỗ Rừng Tinh Linh', price: 1000, type: 'Rèn Đồ', desc: '+20 Sát Thương Phép Magic ATK' },
  ...
  Object.entries(medievalShopCatalog).forEach(([id, item]) => {
    ...
    shopItemsStr += `${icon} **${item.name}** (\`${id}\`): **${formatDong(item.price)}**\n└ *${item.desc}*\n\n`;
  });

  const embed = createDongSonEmbed()
    .setTitle('🛍️ TIỆM THƯƠNG NHÂN TRUNG CỔ (MEDIEVAL SHOP)')
    .setDescription(
      `🏛️ **THƯƠNG NHÂN NPC GOTHIC: Chào mừng ${username}!**\n\n` +
