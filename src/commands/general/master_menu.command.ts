...
export async function masterMenuCommand(message: Message): Promise<void> {
  ...
  if (!validClasses.includes(rawCombat)) {
    const embed = createDongSonEmbed()
      ...
      .setDescription(
        `👋 **Chào mừng ${message.author.username} đến với Thế Giới Medieval Dark Fantasy!**\n\n` +
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId('job_combat_warrior').setLabel('⚔️ Warrior (Kị Sĩ)').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId('job_combat_mage').setLabel('🔮 Mage (Pháp Sư)').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId('job_combat_ranger').setLabel('🏹 Ranger (Cung Thủ)').setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId('job_combat_assassin').setLabel('🗡️ Assassin (Sát Thủ)').setStyle(ButtonStyle.Secondary)
    );
