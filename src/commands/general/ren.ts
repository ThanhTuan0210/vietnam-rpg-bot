...
export async function renCommand(message: Message, args: string[]): Promise<void> {
  ...
  if (rawInput === 'items' || rawInput === 'mat' || rawInput === 'materials' || rawInput === 'nguyenlieu') {
    const embed = createDongSonEmbed()
      ...
      .setDescription(
        `**DANH SÁCH TOÀN BỘ MÃ ID NGUYÊN LIỆU VÀ VÙNG FARM:**\n\n` +
  ...
  if (rawInput === 'sword' || rawInput === 'vukhi' || rawInput === 'kiem') {
    ...
    const bestWeaponRecipe = [...RECIPES]
      .reverse()
      .find((r) => r.requiredLevel <= userLevel && ITEMS[r.resultItemId]?.type === 'vukhi');

    if (!bestWeaponRecipe) {
      await message.reply('❌ Chưa tìm thấy công thức Vũ khí phù hợp với Level của bạn!');
      return;
    }
    const result = await CraftingService.craftItem(userId, bestWeaponRecipe.resultItemId);
    await message.reply(result.message);
    return;
  } else if (rawInput === 'armor' || rawInput === 'aogiap' || rawInput === 'giap') {
    const userLevel = user.canhGioi.capDo;
    const bestArmorRecipe = [...RECIPES]
      .reverse()
      .find((r) => r.requiredLevel <= userLevel && ITEMS[r.resultItemId]?.type === 'aogiap');

    if (!bestArmorRecipe) {
      await message.reply('❌ Chưa tìm thấy công thức Chiến Bào phù hợp với Level của bạn!');
      return;
    }
    const result = await CraftingService.craftItem(userId, bestArmorRecipe.resultItemId);
    await message.reply(result.message);
    return;
  }
  ...
  if (rawInput.length > 0) {
    const result = await CraftingService.craftItem(userId, rawInput);
    await message.reply(result.message);
    return;
  }
  ...
  const groupedRecipes: Record<number, typeof RECIPES> = {};
  for (const r of RECIPES) {
    if (!groupedRecipes[r.requiredLevel]) groupedRecipes[r.requiredLevel] = [];
  ...
  for (const lvl of sortedLevels) {
    if (lvl > userLevel + 15) continue;
    const recipesAtLevel = groupedRecipes[lvl];

    recipeBody += `**Level ${lvl} recipes**\n`;

    for (const r of recipesAtLevel) {
      const itemDef = ITEMS[r.resultItemId] || { name: r.resultItemId, icon: '⚔️', type: 'vukhi', statBonus: {} };
      ...
      const matStr = r.materials
        .map((m) => {
          const matDef = ITEMS[m.itemId] || { name: m.itemId, icon: '📦' };
          return `${matDef.icon} \`${m.itemId}\` x${m.quantity}`;
        })
        .join(' + ');

      const feeStr = r.dongCost > 0 ? ` + ${formatDong(r.dongCost)}` : '';
  ...
  const embed = createDongSonEmbed()
    ...
    .setDescription(
      `• Gõ \`vn craft [mã_vật_phẩm]\` để rèn đồ (Ví dụ: \`vn craft dao_tre_gai\` hoặc \`vn craft sword\`)\n` +
