...
import { UserModelAdvanced, HePhaiType } from '../../database/models/User.model';
...
export async function batDauCommand(message: Message): Promise<void> {
  ...
  if (user.hePhai) {
    const classNames: Record<HePhaiType, string> = {
      DUNG_TUONG: '🛡️ Dũng Tướng (Tanker / Cận chiến)',
      DAO_SI: '🔮 Đạo Sĩ (Phép thuật / Burst)',
      THO_SAN: '🏹 Thợ Săn (Chí mạng / Tốc độ)',
    };
    await message.reply(
      `📜 Bạn đã chọn Hệ Phái **${classNames[user.hePhai]}**! Hãy gõ ` + '`vn nhanvat`' + ` để xem chi tiết thông số nhân vật.`
    );
    return;
  }
  ...
  collector.on('collect', async (i) => {
    const selectedClass = i.customId.replace('class_', '') as HePhaiType;

    await UserModelAdvanced.updateOne({ userId }, { $set: { hePhai: selectedClass } });
    ...
    const resultEmbed = createDongSonEmbed()
      ...
      .setDescription(
        `Chúc mừng bạn đã chọn Hệ Phái **${selectedClass}**! Bạn nhận được **${formatDong(5000)}** khởi đầu cùng bộ vũ khí tân thủ.\n\nGõ \`vn san\` để bắt đầu đi diệt quỷ!`
      );
