...
export class CraftingService {
  ...
  public static async craftItem(
    ...
    const recipe = RECIPES.find((r) => r.resultItemId === targetItemId);
    if (!recipe) {
    ...
    if (user.taiChinh.dong < recipe.dongCost) {
      return {
        success: false,
        message: `❌ Bạn không đủ **${formatDong(recipe.dongCost)}** phí lò rèn!`,
      };
    }
    ...
    return {
      success: true,
      message: `🔨 **CHẾ TẠO THÀNH CÔNG!** Bạn đã đúc thành công ${resultItem.icon} **${resultItem.name} x${recipe.resultQty}** với phí ${formatDong(
        recipe.dongCost
      )}!`,
    };
  }
