...
export class HelpMenuService {
   ...
   */
  public static renderHelpEmbed(prefix = 'vkl') {
    return createDongSonEmbed()
      .setTitle('📜 BẢNG LỆNH GAMER - MEDIEVAL KYRISE RPG (PREFIX: vkl)')
      .setDescription(
        `🎮 **Bảng Điều Khiển Master Menu 1-Click:** Gõ \`vkl\` để bật Bảng Nút Bấm & Dropdown Menu!\n\n`
      )
      .addFields(
        {
          name: '🎮 Phím Tắt 1 Chữ Cho Gamer',
          value:
            `\`vkl\` (Master Menu 1-Click), \`vkl w\` (Combo 5-trong-1), \`vkl h\` (Hunt), ` +
        ...
        },
        {
          ...
          value:
            `\`vkl job sel <war|mag|ran|ass> <min|alc|blk|hnt>\` (Chọn 1 Combat + 1 Producer)\n` +
        ...
        },
        {
          ...
          value:
            `\`vkl vlt dep <id> <qty>\` (Gửi đồ vào Kho Vault Tổ Đội 3-5 bạn bè)\n` +
        ...
        },
        {
          ...
          value:
            '**Tầng 1 (Lv 1-15):** Hang Goblin Rừng Tre (`vkl d 1`)\n' +
