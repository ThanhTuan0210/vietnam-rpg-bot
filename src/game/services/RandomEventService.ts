...
export class RandomEventService {
  public static async tryTriggerEvent(userIdOrMsg: any): Promise<any> {
    if (userIdOrMsg && typeof userIdOrMsg === 'object' && 'channel' in userIdOrMsg) {
      return triggerRandomSurpriseEvent(userIdOrMsg as Message);
    }
    return null;
  }

  public static async triggerRandomEvent(userId: string): Promise<any> {
    return null;
  }
}

export async function triggerRandomSurpriseEvent(message: Message): Promise<boolean> {
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(evt.btnId).setLabel(evt.btnLabel).setStyle(ButtonStyle.Success)
  );
