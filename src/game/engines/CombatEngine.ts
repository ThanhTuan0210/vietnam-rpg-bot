...
export interface CombatStatsTotal {
  totalAtk: number;
  totalDef: number;
  totalMaxHp: number;
  totalMaxMp: number;
...
export interface SkillActionOutput {
  attackerName: string;
  defenderName: string;
  actionName: string;
  damageDealt: number;
...
export interface CombatStateSnapshot {
  ...
  monsterDef: number;
  monsterDodge?: number;
  monsterStatus: { type: string; duration: number; value: number }[];
}

export type CombatState = CombatStateSnapshot;

export class CombatEngineAdvanced {
  /**
   * Tính toán toàn bộ chỉ số tổng hợp của nhân vật (Base + Class + Trang bị + Cường hóa)
   */
  public static calculateTotalStats(user: IUserAdvanced): CombatStatsTotal {
    ...
    if (user.trangBi?.vuKhi && ITEMS[user.trangBi.vuKhi.itemId]) {
      const item = ITEMS[user.trangBi.vuKhi.itemId];
    ...
    if (user.trangBi?.aoGiap && ITEMS[user.trangBi.aoGiap.itemId]) {
      const item = ITEMS[user.trangBi.aoGiap.itemId];
  ...
  public static executeNormalAttack(state: CombatStateSnapshot, monsterName: string) {
  ...
  public static executeMonsterTurn(state: CombatStateSnapshot, monsterName: string) {
  ...
  public static processStatusEffects(state: CombatStateSnapshot) {
  ...
  public static executeClassSkill(
    ...
    monsterName: string
  ): { output: SkillActionOutput; updatedState: CombatStateSnapshot; mpDeducted: number; errorMsg?: string } {
    const classType = (user.hePhai || '').toString().toLowerCase();
