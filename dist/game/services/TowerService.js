...
const TowerSession_model_1 = require("../../database/models/TowerSession.model");
const TowerLeaderboard_model_1 = require("../../database/models/TowerLeaderboard.model");
const User_model_1 = require("../../database/models/User.model");
const CombatEngine_1 = require("../engines/CombatEngine");
const TowerConfig_1 = require("../config/TowerConfig");
class TowerService {
    /**
     * Khởi tạo hoặc đọc phiên leo tháp dở dở của người chơi từ CSDL
     */
    static async startOrResumeRun(userId) {
        let session = await TowerSession_model_1.TowerSessionModel.findOne({ userId, isActive: true });
        if (!session) {
            ...
            session = await TowerSession_model_1.TowerSessionModel.create({
                userId,
                currentFloor: 1,
    ...
    static async executeFloorCombat(userId) {
        ...
        const baseStats = CombatEngine_1.CombatEngineAdvanced.calculateTotalStats(user);
        ...
        session.activeBuffs.forEach((b) => {
            if (b.type === 'ATK_PERCENT')
                atkMult += b.value;
            if (b.type === 'DEF_PERCENT')
                defMult += b.value;
            if (b.type === 'CRIT_RATE')
                critBonus += b.value;
            if (b.type === 'CRIT_DMG')
                critDmgBonus += b.value;
            if (b.type === 'LIFE_STEAL')
                lifeStealBonus += b.value;
            if (b.type === 'REFLECT_DMG')
                reflectRatio += b.value;
            if (b.type === 'MP_RESTORE')
                mpRestoreAmount += b.value;
        ...
        if (isDead) {
            ...
            if (session.currentFloor % 5 === 0 && session.currentFloor % 10 !== 0) {
                ...
                const rolled = TowerConfig_1.TowerConfig.roll3Boons();
     ...
     */
    static async chooseBoon(userId, boonId) {
