...
class CombatEngineAdvanced {
    ...
    static calculateTotalStats(user) {
        ...
        if (user.trangBi.vuKhi && items_1.ITEMS[user.trangBi.vuKhi.itemId]) {
            ...
            if (w.statBonus?.satThuong)
                totalAtk += w.statBonus.satThuong;
            if (user.trangBi.vuKhi.bonusStat)
                totalAtk += user.trangBi.vuKhi.bonusStat;
        }
        ...
        if (user.trangBi.aoGiap && items_1.ITEMS[user.trangBi.aoGiap.itemId]) {
            ...
            if (a.statBonus?.phongThu)
                totalDef += a.statBonus.phongThu;
            if (a.statBonus?.sinhLucToiDa)
                totalMaxHp += a.statBonus.sinhLucToiDa;
            if (user.trangBi.aoGiap.bonusStat)
                totalDef += user.trangBi.aoGiap.bonusStat;
        }
     ...
     */
    static executeNormalAttack(attackerName, defenderName, atk, critRate, defenderDef, defenderDodge) {
        // 1. Kiểm tra Né Tránh
    ...
    static executeClassSkill(user, state, monsterName) {
        ...
        return { output: null, updatedState, mpDeducted: 0, errorMsg: 'Hệ phái không hợp lệ.' };
    }
    /**
     * Xử lý lượt tấn công của Quái / Boss (bao gồm Giai đoạn Cuồng Bạo Enrage)
     */
    static executeMonsterTurn(monster, state) {
        const updatedState = { ...state };
        ...
        const normalAttack = this.executeNormalAttack(monster.ten, 'Bạn', updatedState.monsterAtk, 0.05, updatedState.playerDef, updatedState.playerDodge);
