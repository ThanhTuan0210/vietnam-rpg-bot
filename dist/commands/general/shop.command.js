...
async function shopCommand(message, args) {
    ...
    if (firstArg === 'kimbao' || firstArg === 'kb') {
        ...
        if (!subArg) {
            const embed = (0, embedBuilder_1.createDongSonEmbed)()
                ...
                .setDescription(`**QUAN PHỦ NPC**: Dùng **Kim Bảo** 💎 tích lũy để sở hữu báu vật độc quyền!\n` +
        ...
        if (!item) {
            await message.reply(`❌ Vật phẩm \`${rawItemId}\` không có trong Gian Hàng Kim Bảo!\n` +
        ...
        if (user.taiChinh.kimBao < totalKB) {
            await message.reply(`❌ Bạn không đủ Kim Bảo! Cần **${(0, formatters_1.formatKimBao)(totalKB)}** để mua ${qty}x **${item.name}** (Bạn đang có: **${(0, formatters_1.formatKimBao)(user.taiChinh.kimBao)}**)!`);
            return;
        }
        await User_model_1.UserModelAdvanced.updateOne({ userId }, { $inc: { 'taiChinh.kimBao': -totalKB } });
        await UserService_1.UserService.addItemAtomic(userId, rawItemId, qty);
        const embed = (0, embedBuilder_1.createDongSonEmbed)()
            .setTitle('💎 MUA HÀNG KIM BẢO THÀNH CÔNG!')
            .setDescription(`Bạn đã sử dụng **${(0, formatters_1.formatKimBao)(totalKB)}** để mua thành công ${qty}x **${item.name}** (\`${rawItemId}\`)!`);
        await message.reply({ embeds: [embed] });
        return;
    }
    // =========================================================================
    ...
    if (firstArg === 'mua' || firstArg === 'buy') {
        ...
        if (kimBaoCatalog[rawItemId] && !dongCatalog[rawItemId]) {
            await message.reply(`💎 **VẬT PHẨM BÁN BẰNG KIM BẢO!**\n` +
        ...
        if (rawItemId === 'bua_tru_ta') {
            ...
            const unitPrice = exports.BUA_TRU_TA_REGION_PRICES[region] || 10000;
            const totalCost = unitPrice * qty;
            if (user.taiChinh.dong < totalCost) {
                await message.reply(`❌ Bạn hiện đang ở **Vùng ${region}**. Giá **Bùa Trừ Tà** là **${(0, formatters_1.formatDong)(unitPrice)}/cái** (Tổng: **${(0, formatters_1.formatDong)(totalCost)}**)!`);
                return;
            }
            const success = await UserService_1.UserService.deductDongAtomic(userId, totalCost);
            if (!success) {
                await message.reply('❌ Giao dịch thất bại do không đủ tiền!');
                return;
            }
            await UserService_1.UserService.addItemAtomic(userId, 'bua_tru_ta', qty);
            const embed = (0, embedBuilder_1.createDongSonEmbed)()
                .setTitle('⛩️ MUA BÙA TRỪ TÀ THÀNH CÔNG!')
                .setDescription(`Bạn thuộc **Vùng ${region}** đã chi **${(0, formatters_1.formatDong)(totalCost)}** (Giá: \`${(0, formatters_1.formatDong)(unitPrice)}/cái\`) để mua ${qty}x **Bùa Trừ Tà Tiến Phụ Bản**!`);
            await message.reply({ embeds: [embed] });
            return;
        }
        ...
        if (dongCatalog[rawItemId]) {
            ...
            if (user.taiChinh.dong < totalCost) {
                await message.reply(`❌ Bạn cần **${(0, formatters_1.formatDong)(totalCost)}** để mua ${qty}x **${item.name}**!`);
                return;
            }
            const success = await UserService_1.UserService.deductDongAtomic(userId, totalCost);
            if (!success) {
            ...
            const embed = (0, embedBuilder_1.createDongSonEmbed)()
                ...
                .setDescription(`Bạn đã chi **${(0, formatters_1.formatDong)(totalCost)}** để mua ${qty}x **${item.name}** (\`${rawItemId}\`) cất vào túi đồ!`);
            await message.reply({ embeds: [embed] });
            return;
        }
    ...
    // =========================================================================
    let currentPage = 1;
    const userDoc = await User_model_1.UserModelAdvanced.findOne({ userId });
    const userRegion = userDoc?.canhGioi.khuVuc || 1;
    const buaPriceCurrent = exports.BUA_TRU_TA_REGION_PRICES[userRegion] || 10000;
    const renderPage = (page) => {
        if (page === 1) {
            return (0, embedBuilder_1.createDongSonEmbed)()
                .setTitle(`Tiệm Quan Phủ — Page 1/3 (Trang Bị & Nông Sản)`)
                .setDescription(`**QUAN PHỦ NPC**: Hế lô, **${username}**!\n` +
            ...
            return (0, embedBuilder_1.createDongSonEmbed)()
                ...
                .setDescription(`**QUAN PHỦ NPC**: Hế lô, **${username}**! Mua bằng lệnh \`vn buy [mã_vật_phẩm]\`\n\n` +
                `⛩️ __bua_tru_ta__ | **${(0, formatters_1.formatDong)(buaPriceCurrent)}** *(Giá Vùng ${userRegion})*\n` +
            ...
            return (0, embedBuilder_1.createDongSonEmbed)()
                ...
                .setDescription(`🎆 **CHÀO MỪNG ĐẠI LỄ QUỐC KHÁNH 2/9!** (Vật phẩm giảm giá đặc biệt & Rương Thượng Cổ)\n\n` +
    ...
    collector.on('end', async () => {
        ...
        buildButtons(currentPage).components.forEach((btn) => disabledRow.addComponents(discord_js_1.ButtonBuilder.from(btn).setDisabled(true)));
