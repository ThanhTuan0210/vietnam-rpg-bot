...
exports.CHESTS_BALANCED = void 0;
exports.caoThiCommand = caoThiCommand;
exports.ghepRuongCommand = ghepRuongCommand;
exports.moRuongCommand = moRuongCommand;
const UserService_1 = require("../../game/services/UserService");
const embedBuilder_1 = require("../../utils/embedBuilder");
const formatters_1 = require("../../utils/formatters");
const User_model_1 = require("../../database/models/User.model");
const items_1 = require("../../game/data/items");
exports.CHESTS_BALANCED = {
    ruong_go: {
        id: 'ruong_go',
...
function generateRandomQuestsForUser(userId) {
    ...
    const pool1 = [
        `🪵 **Chặt 20 Gỗ Tre Gai** — Thưởng: ${(0, formatters_1.formatDong)(5000)}, 💎 **2 Kim Bảo**`,
        `⛏️ **Đào 15 Quặng Đồng** — Thưởng: ${(0, formatters_1.formatDong)(6000)}, 💎 **3 Kim Bảo**`,
        `🎣 **Câu 10 Cá Trắm Bến Thôn** — Thưởng: ${(0, formatters_1.formatDong)(4500)}, 💎 **2 Kim Bảo**`,
        `🍃 **Hái 20 Lá Thuốc Nam** — Thưởng: ${(0, formatters_1.formatDong)(4000)}, 💎 **2 Kim Bảo**`,
        `🌾 **Gieo 3 Hạt Giống Điền Trang** — Thưởng: ${(0, formatters_1.formatDong)(7000)}, 💎 **3 Kim Bảo**`,
    ];
    const pool2 = [
        `⚔️ **Hạ 10 Ma Da Sông Hồng (Vùng 1)** — Thưởng: ${(0, formatters_1.formatDong)(10000)}, 📦 **1 Rương Gỗ**`,
        `🐺 **Hạ 8 Sói Rừng U Minh (Vùng 2)** — Thưởng: ${(0, formatters_1.formatDong)(15000)}, 📦 **1 Rương Đồng**`,
        `🦄 **Hạ 5 Kỳ Lân Cổ Thạch (Vùng 3)** — Thưởng: ${(0, formatters_1.formatDong)(25000)}, 🟦 **1 Rương Bạc**`,
        `👑 **Hạ 3 Tướng Quỷ Hắc Ám (Vùng 4)** — Thưởng: ${(0, formatters_1.formatDong)(40000)}, 🔮 **1 Rương Vàng**`,
        `🐉 **Khiêu Chiến Phụ Bản Trùm Vùng 1 Lần** — Thưởng: ${(0, formatters_1.formatDong)(30000)}, 💎 **5 Kim Bảo**`,
    ];
    const pool3 = [
        `🎲 **Thắng 3 ván Bầu Cua Dân Gian** — Thưởng: ⛩️ **1 Bùa Trừ Tà**, ${(0, formatters_1.formatDong)(5000)}`,
        `🎲 **Thắng 2 ván Tài Xỉu Quan Phủ** — Thưởng: 🧪 **1 Thuốc x2 EXP**, ${(0, formatters_1.formatDong)(8000)}`,
...
async function caoThiCommand(message) {
    ...
    const embed = (0, embedBuilder_1.createDongSonEmbed)()
        ...
        .setDescription(`**QUAN PHỦ CÁO THỊ**: Nhiệm vụ tự động đổi mới ngẫu nhiên mỗi **3 Giờ**!\n` +
...
async function ghepRuongCommand(message, args) {
    ...
    const currentChest = exports.CHESTS_BALANCED[chestKey];
    if (!currentChest || !currentChest.nextTier) {
        await message.reply('❌ Loại rương không hợp lệ hoặc đã đạt cấp cao nhất!');
        return;
    }
    const nextChest = exports.CHESTS_BALANCED[currentChest.nextTier];
    ...
    if (user.taiChinh.dong < totalCostDong) {
        await message.reply(`❌ Bạn cần **${(0, formatters_1.formatDong)(totalCostDong)}** để ghép rương!`);
        return;
    }
    if (totalCostKimBao > 0 && user.taiChinh.kimBao < totalCostKimBao) {
        await message.reply(`❌ Bạn cần **${(0, formatters_1.formatKimBao)(totalCostKimBao)}** để ghép rương!`);
        return;
    }
    const consumed = await UserService_1.UserService.consumeItemAtomic(userId, chestKey, requiredCount);
    if (!consumed) {
        await message.reply(`❌ Bạn cần **${requiredCount} ${currentChest.name}** để ghép thành **${qty} ${nextChest.name}**!`);
        return;
    }
    await UserService_1.UserService.deductDongAtomic(userId, totalCostDong);
    if (totalCostKimBao > 0) {
        await User_model_1.UserModelAdvanced.updateOne({ userId }, { $inc: { 'taiChinh.kimBao': -totalCostKimBao } });
    }
    await UserService_1.UserService.addItemAtomic(userId, currentChest.nextTier, qty);
    const embed = (0, embedBuilder_1.createDongSonEmbed)()
        .setTitle('✨ DUNG HỢP RƯƠNG BÁU THÀNH CÔNG!')
        .setDescription(`Bạn đã tiêu tốn **${requiredCount} ${currentChest.name}** + ${(0, formatters_1.formatDong)(totalCostDong)}${totalCostKimBao > 0 ? ` + ${(0, formatters_1.formatKimBao)(totalCostKimBao)}` : ''} để ghép thành **${qty} ${nextChest.icon} ${nextChest.name}**!`);
    await message.reply({ embeds: [embed] });
}
async function moRuongCommand(message, args) {
    const userId = message.author.id;
    ...
    const embed = (0, embedBuilder_1.createDongSonEmbed)().setTitle(`🎉 MỞ ${chest.icon} ${chest.name.toUpperCase()}!`);
    const lootItems = [];
    let dongGained = 0;
    let kbGained = 0;
    if (chestKey === 'ruong_go') {
        ...
        const def1 = items_1.ITEMS[m1] || { name: m1, icon: '📦' };
        ...
        if (Math.random() < 0.2) {
            ...
            const def2 = items_1.ITEMS[m2] || { name: m2, icon: '⛩️' };
        ...
        if (Math.random() < 0.25)
            kbGained = Math.floor(Math.random() * 2) + 1;
        ...
        if (Math.random() < 0.5)
            kbGained = Math.floor(Math.random() * 4) + 2;
    ...
    if (Math.random() < 0.07) {
        ...
        const bonusDef = exports.CHESTS_BALANCED[selectedBonusChest] || { name: selectedBonusChest, icon: '📦' };
        lootItems.push({ itemId: selectedBonusChest, name: `[🍀 CHÍ MẠNG 7%] ${bonusDef.name}`, icon: bonusDef.icon, qty: 1 });
    }
    // Trao thưởng vào CSDL
    if (dongGained > 0)
        await UserService_1.UserService.addDongAtomic(userId, dongGained);
    if (kbGained > 0)
        await User_model_1.UserModelAdvanced.updateOne({ userId }, { $inc: { 'taiChinh.kimBao': kbGained } });
    ...
    const kbStr = kbGained > 0 ? `\n• 💎 **+${(0, formatters_1.formatKimBao)(kbGained)}**` : '';
    embed.setDescription(`Bạn dùng lực mở ${chest.icon} **${chest.name}** và nhận được quà ngẫu nhiên phong phú:\n\n` +
        `• 🪙 **+${(0, formatters_1.formatDong)(dongGained)}**${kbStr}\n` +
        `${lootStr}`);
    await message.reply({ embeds: [embed] });
}
