...
exports.ruleCommand = ruleCommand;
const discord_js_1 = require("discord.js");
const embedBuilder_1 = require("../../utils/embedBuilder");
async function ruleCommand(message) {
    const embed = (0, embedBuilder_1.createDongSonEmbed)()
        .setTitle('📜 THÔNG BÁO QUY TẮC & HƯỚNG DẪN LUẬT CHƠI — MEDIEVAL KYRISE RPG')
        .setDescription(`🏛️ **CHÀO MỪNG ĐẾN VỚI VƯƠNG QUỐC TRUNG CỔ MEDIEVAL KYRISE!**\n` +
    ...
    const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('cmd_master_menu').setLabel('🎮 Bảng Master Menu (vkl)').setStyle(discord_js_1.ButtonStyle.Success), new discord_js_1.ButtonBuilder().setCustomId('cmd_mastery').setLabel('📜 Thông Thạo Nghề (vkl mastery)').setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId('cmd_cooldown').setLabel('⏱️ Bảng Cooldown (vkl cd)').setStyle(discord_js_1.ButtonStyle.Secondary));
