...
async function petCommand(message, args) {
    ...
    if (subCommand === 'adp' || subCommand === 'adopt' || subCommand === 'nhan') {
        ...
        if (!petDef) {
            await message.reply('⚠️ **Linh thú không hợp lệ!** Chọn 1 trong 4 Linh Thú Trung Cổ:\n' +
        ...
        if (user.taiChinh.dong < petDef.price) {
            await message.reply(`⚠️ **Không đủ Tiền Vàng!** Bạn cần **${(0, formatters_1.formatDong)(petDef.price)}** để nhận nuôi ${petDef.icon} **${petDef.name}**.`);
            return;
        }
        user.taiChinh.dong -= petDef.price;
        user.petType = petDef.id;
        user.petLevel = 1;
        await user.save();
        const embed = (0, embedBuilder_1.createDongSonEmbed)()
            .setTitle('🐣 NHẬN NUÔI LINH THÚ THÀNH CÔNG!')
            .setDescription(`🎉 **Chúc mừng ${message.author.username}!** Bạn đã sở hữu Linh Thú Đồng Hành:\n\n` +
    ...
    if (subCommand === 'feed' || subCommand === 'choan') {
        const currentPetId = user.petType;
        ...
        user.petLevel = (user.petLevel || 1) + 1;
        await user.save();
        const petDef = MEDIEVAL_PETS[currentPetId] || { name: currentPetId, icon: '🐾' };
        const embed = (0, embedBuilder_1.createDongSonEmbed)()
            .setTitle('🍖 CHO LINH THÚ ĂN THÀNH CÔNG!')
            .setDescription(`🎉 **${message.author.username}** đã cho ${petDef.icon} **${petDef.name}** ăn 1x Thuốc HP!\n\n` +
            `📈 **Linh Thú đã tăng lên Level ${user.petLevel}!** Sức mạnh hỗ trợ chuyên môn tăng thêm **+5%**!`);
        await message.reply({ embeds: [embed] });
        return;
    }
    // 3. HIỂN THỊ TRẠNG THÁI LINH THÚ CURRENT & DANH SÁCH
    const currentPetId = user.petType || '';
    const currentPetLvl = user.petLevel || 0;
    const activePetDef = MEDIEVAL_PETS[currentPetId];
    let petCatalogStr = '';
    Object.values(MEDIEVAL_PETS).forEach((p) => {
        petCatalogStr += `${p.icon} **${p.name}** (\`${p.id}\`): **${(0, formatters_1.formatDong)(p.price)}**\n└ *Nội tại:* ${p.buffDesc}\n\n`;
    });
    const embed = (0, embedBuilder_1.createDongSonEmbed)()
    ...
    const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('cmd_profile').setLabel('🎒 Hồ Sơ (vkl p)').setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId('cmd_tuido').setLabel('🎒 Túi Đồ (vkl i)').setStyle(discord_js_1.ButtonStyle.Success));
